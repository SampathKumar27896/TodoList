...
<div>
  <form method='post' name='edit_form' onsubmit="return validate_form()">
 ...
 <?php
      ...
      if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_button'])){
             ...
             $sql = "SELECT id,task_name,due_date FROM my_tasks WHERE id='$button_id'";
             ...
             if($connection->query($sql) === TRUE) {
              ...
              $due_date = $row['due_date'];
                echo "<p id='row_id'  style='display: none'>{$row['id']}</p>";
                echo "<p id='task_name' style='display: none'>{$row['task_name']}</p>";
                echo "<p id='due_date' style='display: none'>{$row['due_date']}</p>";
                echo "<script>pass_to_edit();</script>";
      ...
      if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ok_task_edit_button'])){
                ...
                if(!empty($task_name) && !empty($due_date)){
                  ...
                  if(empty($task_name)){
                    echo  "<script>
                        $('#edit_task_box').attr('placeholder','Enter a valid task or date');
                        $('#edit_task_id_box').attr('value', '$row_id');
                        $('#date_set').attr('value','$today')
                        </script>";
                  }
